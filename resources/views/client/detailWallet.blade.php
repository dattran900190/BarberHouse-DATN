                        ...
                        <div class="card-header border-bottom-dashed p-4">
                            <div class="d-sm-flex justify-content-between align-items-center">
                                ...
                                <div class="flex-shrink-0 mt-sm-0 mt-3">
                                    <a href="{{ route('client.wallet') }}" class="btn btn-outline-primary btn-sm">
                            ...
                            <div class="row g-3 mb-4">
                                    ...
                                    <form method="GET" action="{{ route('client.detailWallet') }}" id="searchForm">
                                        <div class="position-relative">
                                            <input type="text" name="search" class="form-control"
                                                placeholder="Tìm kiếm theo mã đơn hàng hoặc mã đặt lịch"
                                ...
                                <div class="col-lg-4 col-md-5">
                                    @php
                                        ...
                                        // Lấy nhãn hiển thị tương ứng, mặc định là 'Lọc theo trạng thái'
                                        $displayLabel = $statusLabels[$currentStatus] ?? 'Lọc theo trạng thái';
                                    @endphp
                                    <div class="dropdown">
                            ...
                            @forelse ($refunds as $refund)
                                    ...
                                    <div class="row g-3 align-items-center">
                                        <div class="col-lg-7 col-md-12">
                                            ...
                                            @if ($refund->refund_status === 'rejected' && $refund->reject_reason)
                                                <div class="mb-2">
                                                    <span class="text-muted">Lý do từ chối:
                                                        {{ $refund->reject_reason }}</span>
                                                </div>
                                            @endif
                                            @if ($refund->refund_status === 'refunded' && $refund->proof_image)
                                                <div class="mb-2">
                                                    <span class="text-muted">Hình ảnh minh chứng:
                                                        <a href="{{ Storage::url($refund->proof_image) }}"
                                                            target="_blank">Xem hình ảnh</a></span>
                                                </div>
                                            @endif
                                        </div>
                                ...
                                <div class="collapse mt-3" id="detail-{{ $refund->id }}">
                                    <div class="card card-body bg-light">
                                        <div class="row g-3">
                                            <div class="col-md-6">
                                                <h6 class="text-muted text-uppercase fw-semibold fs-15">Thông tin ngân hàng
                                                </h6>
                                                <p class="text-muted mb-1">{{ $refund->bank_name }} -
                                                    {{ $refund->bank_account_number }}</p>
                                                <p class="text-muted mb-1">Chủ tài khoản: {{ $refund->bank_account_name }}
                                                </p>
                                            </div>
                                            <div class="col-md-6">
                                                <h6 class="text-muted text-uppercase fw-semibold fs-15">Thông tin yêu cầu
                                                </h6>
                                                <p class="text-muted mb-1">Lý do: {{ $refund->reason }}</p>
                                                <p class="text-muted mb-1">Số tiền:
                                                    {{ number_format($refund->refund_amount, 0, ',', '.') }} VNĐ</p>
                                                <p class="text-muted mb-1">Ngày yêu cầu:
                                                    {{ $refund->created_at->format('d/m/Y H:i') }}</p>
                                                @if ($refund->refund_status === 'refunded' && $refund->refunded_at)
                                                    <p class="text-muted mb-1">Ngày hoàn tiền:
                                                        {{ $refund->refunded_at->format('d/m/Y H:i') }}</p>
                                                @endif
                                                @if ($refund->refund_status === 'rejected' && $refund->reject_reason)
                                                    <p class="text-muted mb-1">Lý do từ chối: {{ $refund->reject_reason }}
                                                    </p>
                                                @endif
                                            </div>
                                        </div>
                                        @if ($refund->refund_status === 'refunded' && $refund->proof_image)
                                            <div class="mt-3">
                                                <h6 class="text-muted text-uppercase fw-semibold fs-15">Hình ảnh minh chứng
                                                </h6>
