            ...
            <form method="POST" action="{{ route('cart.checkout.process') }}" class="mainCheckout">
                ...
                <div class="informationProduct">
                    ...
                    <div class="form-check" style="display: flex; justify-content: space-between">
                        <div class="chon">
                            <input class="form-check-input" value="1" type="radio" name="phuong_thuc_thanh_toan_id" id="paymentMethodCOD" checked />
                            <label class="form-check-label" for="paymentMethodCOD">Thanh toán khi giao hàng (COD)</label>
                        </div>
                        <div class="icon-bank">
                    ...
                    <div class="form-check" style="display: flex; justify-content: space-between">
                        <div class="chon">
                            <input class="form-check-input" value="2" type="radio" name="phuong_thuc_thanh_toan_id" id="paymentMethodVNPAY" />
                ...
                <div class="informationProduct">
                        ...
                        <tbody>
                            ...
                            @foreach ($items as $item)
                                @php
                                    ...
                                    $total += $subtotal;
                                @endphp
                                @foreach (['cart_item_id', 'id', 'name', 'price', 'quantity', 'image'] as $field)
                                    <input type="hidden" name="items[{{ $loop->parent->index }}][{{ $field }}]" value="{{ $item[$field] }}" />
                                @endforeach
                    ...
                    <div class="tong-cong">
                        <div class="tong-thanh-toan">
                            ...
                            <input type="hidden" id="total-input" name="tong_tien" value="{{ $total + $shippingFee }}" />
                            <h5><span id="total-display">{{ number_format($total + $shippingFee, 0, ',') }}</span> VNĐ</h5>
                        </div>
                    </div>

                    <div class="dat-hang">
                        <a href="{{ route('cart.show') }}"><i class="fa-solid fa-angle-left"></i> Quay về giỏ hàng</a>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        radios.forEach(radio => {
            radio.addEventListener('change', function() {
                ...
                totalDisplay.textContent = total.toLocaleString('vi-VN', { maximumFractionDigits: 0 });
            });
        });
    });
</script>
